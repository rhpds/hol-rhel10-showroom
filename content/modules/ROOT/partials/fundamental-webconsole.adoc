...
NOTE:  Please note that by default, the webconsole is configured to use port 9090 as part of the local server URL.  In this exercise the Web Console browser access is embedded in a tab with no need to open a separate browser or window.

To access the Web Console, navigate to the "Cockpit" Tab, and login with the following UserID and Password:
...
====
image::rhel-10.0/wc-001.png[Cockpit Login,link=self,window=blank]
====
...
====
image::rhel-10.0/wc-002.png[Cockpit Overview,link=self,window=blank]
====

Please proceed to enable administrative access and then verify.

====
image::rhel-10.0/wc-003.png[Cockpit Overview,link=self,window=blank]
====
...
====
image::rhel-10.0/wc-004.png[View details and history,link=self,window=blank]
====

Next, we need to enable the collection of metrics data.

====
image::rhel-10.0/wc-005.png[Metrics Settings,link=self,window=blank]
====

====
image::rhel-10.0/wc-006.png[Enable Metrics,link=self,window=blank]
====
...
====
image::rhel-10.0/wc-007.png[Performance Charts,link=self,window=blank]
====
...
====
image::rhel-10.0/wc-008.png[Cockpit Perf Profile,link=self,window=blank]
====

A dialog box will appear.  Scroll and find "throughput-performance" and select.

====
image::rhel-10.0/wc-009.png[Cockpit Perf Throughout,link=self,window=blank]
====
...
====
image::rhel-10.0/wc-010.png[Cockpit Logs,link=self,window=blank]
====
...
====
image::rhel-10.0/wc-011.png[Cockpit Logs Criteria,link=self,window=blank]
====
...
====
image::rhel-10.0/wc-012.png[Cockpit Network,link=self,window=blank]
====
...
====
image::rhel-10.0/wc-013.png[Cockpit Firewall,link=self,window=blank]
====

Now you should see a list of active services and ports.  Proceed to select "Add services"

====
image::rhel-10.0/wc-014.png[Cockpit Firewall Service,link=self,window=blank]
====

In the dialog box enter 'ntp' as the filter, select 'ntp' and hit "Add services".

====
image::rhel-10.0/wc-015.png[Cockpit Firewall Dialog,link=self,window=blank]
====
...
[NOTE]
====
The Service Selector Filter does allow searching for either "chrony" or "ntp" but when searching in tthe Firewall service, it only recognizes "ntp" and not "chrony".
====

====
image::rhel-10.0/wc-016.png[Cockpit Services,link=self,window=blank]
====
...
====
image::rhel-10.0/wc-017.png[Cockpit Services,link=self,window=blank]
====
...
A new feature to the Web Console is the 'File Browser'.  This tool allows you to navigate the filesystems (with appropriate permissions of course), make changes and even edit simple files.

So let's take a quick look at some of its capabilities.


====
image::rhel-10.0/wc-018.png[Cockpit File Browser,link=self,window=blank]
====

Change your directory to /var/tmp/ either by navigating or editing the shown path.

====
image::rhel-10.0/wc-019.png[Cockpit File Browser,link=self,window=blank]
====

Change the view to a 'detailed list' instead of the 'icon view', or maybe an alternative sort order.

====
image::rhel-10.0/wc-020.png[Cockpit File Browser,link=self,window=blank]
====

Explore actions you can take on the current directory.

====
image::rhel-10.0/wc-021.png[Cockpit File Browser,link=self,window=blank]
====

Explore the actions you can take by right-clicking on a file.

====
image::rhel-10.0/wc-022.png[Cockpit File Browser,link=self,window=blank]
====

Using a right click, select 'Open Text File' and try editing the provided sample file 'webconsole-sample.txt'.

====
image::rhel-10.0/wc-023.png[Cockpit File Browser,link=self,window=blank]
====

You can 'Save' any change or simply 'Close' or 'Cancel' to discard any changes.  Either way, proceed to close the editor dialog.
...
====
image::rhel-10.0/wc-024.png[Cockpit Applications,link=self,window=blank]
====
