...
So for this exercise, we use something called a 'Bind-Mount' to effectively mirror a portion of the host's filesystem into the container's namespace.  A Bind-Mount is declared using the '-v' option.  In the example below, /usr/bin and /usr/lib64 from the host will be exposed and accessible to the containers namespace mounted at '/usr/bin' (ie: /usr/bin:/usr/bin).
...
Notice above how there is now a dedicated mount point for /usr/bin.  Bind-Mounts are an effective diagnostic tool to temporarily inject tools and files that are not normally part of a container image.  Remember, using Bind-Mounts as a design decision for enterprise container strategies is folly.
...
localhost/custom_image                latest      162395043b06  3 seconds ago  322 MB
registry.access.redhat.com/ubi10/ubi  latest      a6ad0cc3cdc0  3 days ago     219 MB
----
...
... SNIP ...
└─machine.slice
  ├─libpod-conmon-ba12c95cddd936d93a77be8f8910380716293bf7b743e306474feed4b51bf13a.scope …
  │ └─11604 /usr/bin/conmon --api-version 1 -c ba12c95cddd936d93a77be8f8910380716293bf7b743e306474feed4b51bf13a -u ba12c95cdd…
  └─libpod-ba12c95cddd936d93a77be8f8910380716293bf7b743e306474feed4b51bf13a.scope …
    └─container
      ├─11606 /usr/sbin/httpd -D FOREGROUND
      ├─11608 /usr/sbin/httpd -D FOREGROUND
      ├─11609 /usr/sbin/httpd -D FOREGROUND
      ├─11618 /usr/sbin/httpd -D FOREGROUND
      └─11652 /usr/sbin/httpd -D FOREGROUND
----
