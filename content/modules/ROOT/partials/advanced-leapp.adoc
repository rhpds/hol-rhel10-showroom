...
For these exercises, you will be using the host `leapp` as user `root`.  Before you begin, confirm the node was provisioned with RHEL 9.7
...
NOTE: The Leapp process has explicitly set the release version to "10.1".  This means that updates will only be available and applied for the RHEL 10.1 version.  This can be modified to a more general version "10" in order to receive ongoing updates as more point releases of RHEL 10 come out using either of the following:
...
====
image::rhel-10.0/leapp-02.png[Sample Leapp PreUpgrade Web Console Report,link=self,window=blank]
====
